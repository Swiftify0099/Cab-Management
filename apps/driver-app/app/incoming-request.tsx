...
export default function IncomingRequestScreen({ request, onDismiss }: Props) {
  ...
  const normalizedOffer: RideOfferPayload = {
    ...
    trip: {
      ...
      distance_km: Number(request?.trip?.distance_km ?? request?.distance_km) || 12.8,
      duration_min: Number(request?.trip?.duration_min ?? request?.duration_min) || 28,
      fare: Number(request?.trip?.fare ?? request?.fare) || 285,
      earning: Number(request?.trip?.earning ?? request?.earning) || Math.round((Number(request?.fare) || 285) * 0.8),
      seats: Number(request?.trip?.seats ?? request?.seats) || 1,
    },
