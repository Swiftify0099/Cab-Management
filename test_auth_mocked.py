...
from backend.common.models.all_models import User, UserRole, AdminProfile
...
mock_admin_profile = AdminProfile(
    ...
    password_hash=ADMIN_PASSWORD, # we will mock verify_password to return True
...
async def override_get_db():
    ...
    # Mocking execute based on what is being queried
    async def mock_execute(query):
        mock_result = MagicMock()
        
        # Determine what's being queried by converting to string
...
async def run_tests():
    ...
    async with AsyncClient(transport=ASGITransport(app=app), base_url="http://test") as client:
        ...
        # Mock Redis methods
        with patch('backend.auth_service.app.api.v1.auth.increment_otp_requests', new_callable=AsyncMock) as mock_inc, \
             patch('backend.auth_service.app.api.v1.auth.store_otp', new_callable=AsyncMock) as mock_store, \
             patch('backend.auth_service.app.api.v1.auth.get_otp', new_callable=AsyncMock) as mock_get_otp, \
             patch('backend.auth_service.app.api.v1.auth.delete_otp', new_callable=AsyncMock) as mock_del_otp:
        ...
        with patch('backend.auth_service.app.api.v1.admin_auth.verify_password', return_value=True):
